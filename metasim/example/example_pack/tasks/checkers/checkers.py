...
class DetectedChecker:
    ...
    def check(self, handler: BaseSimHandler, states: TensorState) -> torch.BoolTensor:
        ...
        if not hasattr(self, "_first_check"):
            self._first_check = torch.ones(handler.num_envs, dtype=torch.bool, device=handler.device)
        if not hasattr(self, "_ignore"):
            self._ignore = torch.zeros(handler.num_envs, dtype=torch.bool, device=handler.device)
