...
class GenesisHandler(BaseSimHandler):
    ...
    def launch(self) -> None:
        ...
        if scene_cfg is not None:
            scene_file = None
            if hasattr(scene_cfg, "file_name"):
                try:
                    scene_file = scene_cfg.file_name(self.scenario.simulator)
                except Exception:
                    scene_file = None
            if scene_file is None:
                scene_file = getattr(scene_cfg, "mjcf_path", None) or getattr(scene_cfg, "urdf_path", None)

            if scene_file is not None:
                scene_scale = getattr(scene_cfg, "scale", 1.0)
                if isinstance(scene_scale, (tuple, list)):
                    scene_scale = float(scene_scale[0])

                if str(scene_file).endswith(".xml"):
                    self.scene_inst.add_entity(
                        gs.morphs.MJCF(file=scene_file, scale=scene_scale, convexify=False, decimate=False)
                    )
                    has_custom_scene = True
                elif str(scene_file).endswith(".urdf"):
                    self.scene_inst.add_entity(gs.morphs.URDF(file=scene_file, fixed=True, scale=scene_scale))
                    has_custom_scene = True
                else:
                    log.warning(f"Unsupported Genesis scene asset format: {scene_file}")

        ## Add ground
        if not has_custom_scene:
            try:
