...
class MujocoHandler(BaseSimHandler):
    ...
    def launch(self) -> None:
        ...
        if not self.headless:
            try:
                ...
                self.viewer = None
                self.headless = True
                log.warning(f"MuJoCo viewer launch failed ({e}). Falling back to headless mode.")
    ...
    def _add_ground(self, mjcf_model: mjcf.RootElement) -> None:
        ...
        elif self.scenario.scene is not None:
            # Loaded scenes can already define floor geometry/materials. Adding the default
            # ground again causes asset-name collisions (for example LIBERO scene texplane).
            return
        else:
